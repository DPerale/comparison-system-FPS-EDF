...
package body System.Tasking.Protected_Objects.Multiprocessors is
   ...
   --     --  We have to signal that the caller task is ready to be
   --     --  rescheduled, but we are not allowed modify the ready queue of the
   --     --  other CPU. We use the Served_Entry_Call list and a poke interrupt
   --     --  to signal that the task is ready.

   --     --  Disabled IRQ ensure atomicity of the operation. Atomicity plus
   --     --  Fair locks ensure bounded execution time.
