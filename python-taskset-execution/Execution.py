...
def U_100_hyper_113400000_10_100_full():
    ...
    for i in range(2, 501):
        ...
        retry = 0
        taskset = []
        taskset, utilization, EDF_busy_period, FPS_busy_period, EDF_first_DM, EDF_schedulable, FPS_schedulable, hyperperiod = import_taskset(taskset, i, "U_100_hyper_113400000_10_100.csv")
        base_hyperperiod = int(hyperperiod)
        while not good:

            if retry == 0:
                with open('../workspace2/U_100_hyper_113400000_10_100/U_100_hyper_113400000_10_100_'+str(i)+'.csv') as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=';')
                    numrow = 0
                    for row in csv_reader:
                ...
                with open('../workspace2/U_100_hyper_113400000_10_100_full/U_100_hyper_113400000_10_100_full_' + str(i)+'.csv') as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=';')
                    good = True
                    numrow = 0
                    for row in csv_reader:
                        if numrow > 0 and numrow <= 20:
                            if int(row[10]) > int(base_hyperperiod/int(row[2])):
                                hyperperiod = int(hyperperiod) - int(float(row[2]) * float(row[12])) - 56
                                good = False
                        numrow = numrow+1
                if not good:
                    make_adb_file(taskset, hyperperiod)
